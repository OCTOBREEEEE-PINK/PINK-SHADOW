...
echo "🔧 Test d'inscription frontend"
echo "============================="

echo "📋 Simulation de l'inscription complète depuis le frontend..."
...
SESSION_RESPONSE=$(curl -s "http://localhost:8000/sessions/DEFAULT/players?pseudo=Joueur1")
...
if [ "$PLAYERS_COUNT" -eq 5 ]; then
    ...
    MISSION_RESPONSE=$(curl -s -X POST "http://localhost:8000/sessions/DEFAULT/start-mission?pseudo=Joueur1" \
        -H "Content-Type: application/json")
...
echo "  - Test de connexion WebSocket pour Joueur1..."
